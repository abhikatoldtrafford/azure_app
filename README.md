...
- `output_format` (optional): "csv" or "excel" for structured data
...
#### `GET /conversation`
Chat with streaming responses (SSE format).
...
```bash
curl "https://copilotv2.azurewebsites.net/download-chat?session=thread_xyz789&assistant=asst_abc123"
```
...
# 3. Analyze the data
curl "https://copilotv2.azurewebsites.net/chat?session=$SESSION&assistant=$ASSISTANT&prompt=What are the top 5 products by revenue?"
...
for img in images/*.jpg; do
  ...
  curl -s -X POST https://copilotv2.azurewebsites.net/completion \
    ...
    -F "files=@$img" \
    | jq -r .response >> analysis_report.txt
done
...
# Test file system
curl https://copilotv2.azurewebsites.net/test-download

# Verify specific endpoint (non-production only)
curl -X POST https://copilotv2.azurewebsites.net/test-endpoint \
  -F "endpoint=/completion" \
  -F "test_data=Hello world"
```

## 📞 Support

For issues, feature requests, or contributions:
1. Check the [health status](https://copilotv2.azurewebsites.net/health-check)
